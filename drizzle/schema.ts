import { pgTable, index, foreignKey, varchar, bigint, numeric, timestamp, text, date, unique, boolean, integer, pgEnum } from "drizzle-orm/pg-core"
...
export const gameGenres = pgTable("game_genres", {
	...
	index("idx_game_genres_genre_name").using("btree", table.genreName.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.gameId],
			foreignColumns: [games.id],
...
export const gamePlatforms = pgTable("game_platforms", {
	...
	index("idx_game_platforms_platform_name").using("btree", table.platformName.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.gameId],
			foreignColumns: [games.id],

...
export const userGameStatus = pgEnum("user_game_status", ['backlog', 'playing', 'played', 'dropped'])
export const userRole = pgEnum("user_role", ['user', 'supporter', 'admin'])


export const howlongtobeatData = pgTable("howlongtobeat_data", {
	...
	mainStorySidesHours: numeric("main_story_sides_hours", { precision: 6, scale:  2 }),
	completionistHours: numeric("completionist_hours", { precision: 6, scale:  2 }),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_howlongtobeat_data_completionist_hours").using("btree", table.completionistHours.asc().nullsLast().op("numeric_ops")),
	index("idx_howlongtobeat_data_hltb_id").using("btree", table.hltbId.asc().nullsLast().op("int8_ops")),
	index("idx_howlongtobeat_data_main_story_hours").using("btree", table.mainStoryHours.asc().nullsLast().op("numeric_ops")),
	index("idx_howlongtobeat_data_main_story_sides_hours").using("btree", table.mainStorySidesHours.asc().nullsLast().op("numeric_ops")),
	foreignKey({
...
export const accounts = pgTable("accounts", {
	...
	index("idx_accounts_account_id").using("btree", table.accountId.asc().nullsLast().op("text_ops")),
...
export const games = pgTable("games", {
	...
	index("idx_games_igdb_id").using("btree", table.igdbId.asc().nullsLast().op("int8_ops")),
	index("idx_games_title").using("btree", table.title.asc().nullsLast().op("text_ops")),
...
export const reviews = pgTable("reviews", {
	...
	index("idx_reviews_game_created").using("btree", table.gameId.asc().nullsLast().op("text_ops"), table.createdAt.desc().nullsLast().op("text_ops")),
	index("idx_reviews_user_rating").using("btree", table.userId.asc().nullsLast().op("text_ops"), table.rating.desc().nullsLast().op("numeric_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const usersSocialLinks = pgTable("users_social_links", {
	id: varchar({ length: 36 }).primaryKey().notNull(),
	userId: varchar("user_id", { length: 36 }).notNull(),
...
export const collectionGames = pgTable("collection_games", {
	...
	unique("collection_games_collection_game_unique").on(table.collectionId, table.gameId),
]);
...
export const genres = pgTable("genres", {
	...
	index("idx_genres_slug").using("btree", table.slug.asc().nullsLast().op("text_ops")),
...
export const platforms = pgTable("platforms", {
	...
	index("idx_platforms_slug").using("btree", table.slug.asc().nullsLast().op("text_ops")),
...
export const userGames = pgTable("user_games", {
	...
	index("idx_user_games_rating").using("btree", table.rating.asc().nullsLast().op("numeric_ops")),
	index("idx_user_games_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],

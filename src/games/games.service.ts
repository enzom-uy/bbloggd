...
export class GamesService {
    ...
    private async getGameCompanies(involvedCompanies: number[]) {
        ...
        const companyPromises = involvedCompanies.map((companyId) =>
            this.gameUtilsService.getGameInvolvedCompanies(`${companyId}`),
        )
    ...
    async getGameById(gameId: string): Promise<{
        ...
        if (gameInDb.length === 0) {
            ...
            const gameReleaseDate = this.gameUtilsService.getGameReleaseDate(
    ...
    async getGameSearchSuggestions(
        ...
        const apiCalls = await Promise.allSettled([
            ...
            }).then((res) => res.json() as Promise<IGDBGame[]>),
        ])
        ...
        const uniqueGames = Array.from(gameMap.values()) as Array<{
            igdbId: number
            name: string
        }>
